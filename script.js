...
文法：${grammarList}`,
  ...
  const staticRQ = { passage: null, questions: [], idx: 0, score: 0, correct: 0 };

  function startStaticReadingQuiz(pi) {
    const p = (window.readingPassages || [])[pi];
    if (!p) return;

    // Shuffle all questions and pick up to 5
    const allQs = (p.questions || []).slice();
    for (let i = allQs.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [allQs[i], allQs[j]] = [allQs[j], allQs[i]];
    }
    const picked = allQs.slice(0, Math.min(5, allQs.length));

    // Shuffle options for each question
    const questions = picked.map(q => {
      const opts = q.opts.slice();
      const correctOpt = opts[q.ans];
      // Shuffle opts
      for (let i = opts.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [opts[i], opts[j]] = [opts[j], opts[i]];
      }
      const newAns = opts.indexOf(correctOpt);
      return { ...q, opts, ans: newAns };
    });

    staticRQ.passage = p;
    staticRQ.questions = questions;
    staticRQ.idx = 0;
    staticRQ.score = 0;
    staticRQ.correct = 0;
  ...
  function renderStaticQ() {
    ...
    const q = staticRQ.questions[qi];
    const total = staticRQ.questions.length;
  ...
  function nextStaticQ() {
    staticRQ.idx++;
    if (staticRQ.idx >= staticRQ.questions.length) {
      showOnly("viewReadingResult");
      const score = staticRQ.score;
      const total = staticRQ.questions.length * 10;
      ...
      if (note) note.textContent = `答對 ${staticRQ.correct} / ${staticRQ.questions.length} 題（${pct}%）`;
    } else {
      renderStaticQ();
    }
  }
  ...
  function explainStaticQ() {
    const q = staticRQ.questions[staticRQ.idx];
